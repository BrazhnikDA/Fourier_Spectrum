...
def change_filter(flag):
    if flag == 1:
        ...
        cv2.circle(filter_mat, (np.int32(ncols/2), np.int32(nrows/2)) , d0, (1,1,1), -1)
    elif flag == 2:
        n = 2
        filter_mat = None
...
cv2.imshow("The perfect low pass filter", img1)
cv2.imshow("Butterworth Low Pass Filter", img2)
cv2.imshow("Gaussian low-pass filtering", img3)
cv2.waitKey()